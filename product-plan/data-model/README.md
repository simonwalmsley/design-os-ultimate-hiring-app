...
The "nouns" of Ultimate Hiring Process. These definitions are intentionally minimal - they establish entity names and relationships and leave room for implementation detail (timestamps, soft deletes, audit fields, etc.).
...
## Notes on Section Types

Each section ships its own `types.ts` shaped for that section's UI. Field names vary slightly between sections (for example, a user's name is `fullName` in Authentication and `name` in Admin). Treat `data-model/types.ts` as the canonical model and reconcile section-level shapes against it when wiring up your backend.
