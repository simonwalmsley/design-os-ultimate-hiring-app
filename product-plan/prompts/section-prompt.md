...
- **SECTION_NAME** = [Human-readable name, e.g. "Hiring Threads" or "Rankings Dashboard"]
- **SECTION_ID** = [Folder name in sections/, e.g. "hiring-threads" or "rankings-dashboard"]
- **NN** = [Milestone number - sections start at 02 since 01 is Foundation: 02 authentication-and-onboarding, 03 hiring-threads, 04 conversational-phases, 05 rankings-dashboard, 06 admin]
...
1. **Authentication & Authorization** (if not yet established)
   ...
   - What permissions are needed for this section? (note: the Admin section is staff-only; client sections are scoped to the user's Account)

2. **Data Relationships**
   - How does this section's data relate to other entities?
   - Are there cross-section dependencies? (for example, Conversational Phases and Rankings both live inside a Hiring Thread; Rankings uses the Scoring Categories from Phase 3; the AI model per phase comes from Admin AI Configuration)
...
4. **Backend Business Logic**
   ...
   - Background processes, notifications, or AI jobs to trigger?
